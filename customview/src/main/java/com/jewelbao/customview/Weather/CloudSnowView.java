...
public class CloudSnowView extends SkyconView
{
	...
	PathPoints[] pathPoints11, pathPoints12, pathPoints21, pathPoints22, pointsCircle11, pointsCircle12, pointsCircle21, pointsCircle22;

	private int screenW, screenH;
	private float X, Y;

	private Path path11, path12, path13, path21, path22, path23, //visible drawn paths

	cubicPath11, cubicPath12, cubicPath21, cubicPath22, //Invisible paths for drop movement

	pathCircle1, pathCircle2; //Invisible paths for rotate operation

	int m = 0, n = 0, x1 = 0, y1 = 0, x2 = 0, y2 = 0;

	boolean drop11 = true, drop12 = false, drop21 = false, drop22 = false, pointsStored = false;
	...
	public void onDraw(Canvas canvas)
	{
		...
		float P1Y = ((float) ((int) (0.1041667 * screenW) * Math
				.sin(Math.toRadians(80 + (0.111 * count))) + Y));
		float P2Y = ((float) (((int) (0.1041667 * screenW) + ((0.00023125 * screenW) * count)) * Math
				.sin(Math.toRadians(120 + (0.222 * count))) + Y));
		...
		if(isAnimated && isStatic)
		{
			...
			pathCircle2.addCircle(pathPoints12[x].getX(), pathPoints12[x].getY(), screenW * 0.03f,
								  Path.Direction.CW);
			...
			pathCircle2 = new Path();
			pathCircle2.addCircle(pathPoints22[y].getX(), pathPoints22[y].getY(), screenW * 0.03f,
								  Path.Direction.CW);
		...
		} else
		{
			...
			if(drop11)
			{
				pathCircle1 = new Path();
				pathCircle1
						.addCircle(pathPoints11[m].getX(), pathPoints11[m].getY(), screenW * 0.03f,
								   Path.Direction.CW);
			...
			if(drop12)
			{
				pathCircle2 = new Path();
				pathCircle2
						.addCircle(pathPoints12[n].getX(), pathPoints12[n].getY(), screenW * 0.03f,
								   Path.Direction.CW);
				pointsCircle12 = getPoints(pathCircle2);
			...
			if(drop21)
			{
				pathCircle1 = new Path();
				pathCircle1
						.addCircle(pathPoints21[m].getX(), pathPoints21[m].getY(), screenW * 0.03f,
								   Path.Direction.CW);
			...
			if(drop22)
			{
				pathCircle2 = new Path();
				pathCircle2
						.addCircle(pathPoints22[n].getX(), pathPoints22[n].getY(), screenW * 0.03f,
								   Path.Direction.CW);
				pointsCircle22 = getPoints(pathCircle2);
