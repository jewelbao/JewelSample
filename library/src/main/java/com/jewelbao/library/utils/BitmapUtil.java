...
public class BitmapUtil {
	...
	public static Bitmap getRoundCornerImage(Bitmap bitmap_bg, Bitmap bitmap_in, int width, int height) {
		Bitmap roundConcernImage = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
		Canvas canvas = new Canvas(roundConcernImage);
	 ...
	 * 仿微信聊天图片显示
	 */
	public static void showImage(final Context context, final ImageView iv, final Bitmap image, final int bgResID, final int width, final int heigth) {
		new Thread(new Runnable() {
			@Override
			public void run() {
				Bitmap bitmap_bg = BitmapFactory.decodeResource(context.getResources(), bgResID);
				final Bitmap bp = getRoundCornerImage(bitmap_bg, image, width, heigth);
				final Bitmap bp2 = getShardImage(bitmap_bg, bp, width, heigth);
				((Activity) context).runOnUiThread(new Runnable() {
					@Override
					public void run() {
						iv.setImageBitmap(bp2);
					}
				});
			}
		}).start();
	...
	public static String bitmap2Base64(String filePath) {
		...
		bm.compress(Bitmap.CompressFormat.JPEG, 40, byteArrayOs);
	...
	public static Bitmap getSmallBitmap(String filePath) {
		...
		options.inSampleSize = calculateInSampleSize(options, 480, 800).inSampleSize;
	...
	public static void saveBitmap(String savePath, Bitmap bm) {
		JLog.i("save bitmap start and savePath=" + savePath);
		...
		try {
			...
			out.flush();
			out.close();
			JLog.i("save bitmap success");
			return;
		} catch(IOException e) {
			e.printStackTrace();
		}
		JLog.i("save bitmap fail");
	}
	...
	public static Bitmap blurFast(Context context, Bitmap bm, int radius) {
